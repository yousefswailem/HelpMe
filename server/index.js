  ...
  assignedShops.forEach((driverData, driverId) => {
    driverData.shops.forEach((shop, index) => {
      closestDrivers.push({
        ...
        shop: shop.name,
        distance: distanceData.find(
          (data) => data.shopId === shop.id && data.driverId === driverId
        ).distance,
      });
    });
  });
...
app.get('/closest-drivers', async (req, res) => {
  ...
  const shopLocations = [
    {
      id: 8,
      name: 'BBQ Pizza - Al-Bireh',
      lat: 31.9197688227728,
      lng: 35.21693786802346,
    },
  ...
  const closestDrivers = await findClosestDrivers(driverLocations, shopLocations);
